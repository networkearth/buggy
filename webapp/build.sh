...
if [[ $1 = "dev" ]]
then
docker build --build-arg APP_NAMESPACE=buggy --build-arg APP_ENVIRONMENT=dev --build-arg APP_ACCOUNT=575101084097 --build-arg APP_REGION=us-east-1 --build-arg API_URL=http://buggy.dev.api.networkearth.io --platform linux/amd64 -t buggy-dev-webapp .
...
then
docker build --build-arg APP_NAMESPACE=buggy --build-arg APP_ENVIRONMENT=dev --build-arg APP_ACCOUNT=575101084097 --build-arg APP_REGION=us-east-1 --build-arg API_URL=http://buggy.dev.api.networkearth.io --platform linux/amd64 -t buggy-local-webapp .
